...
Handler.sendSubsaidditPosts = function (req,res){
	...
	var query = database.getConnection().query("SELECT title,SUM(rating) AS 'rating',url FROM posts LEFT JOIN post_ratings ON posts.id = post_ratings.post WHERE subsaiddit=? GROUP BY title ORDER BY rating DESC",req.params.subsaiddit, function(error,results,fields){
    if (error){
      console.log(error);
    } else {
      //console.log(results);
