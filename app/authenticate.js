...
Authenticate = (function() {
  ...
  Authenticate.ensureLoggedIn = function(req, res, next) {
    ...
    if(req.cookies != null){
      if (req.cookies[config.cookieName] != null) {
        return utils.verifySessionToken(req.cookies[config.cookieName], function(err,accountID, accountName, yexpiry) {
          if (err)
          {
 ...
 login = function (data, callback) {
  ...
  database.getConnection().query("SELECT password, id FROM accounts WHERE username = ?", [username], function(error, results, fields) {
    ...
    if (results.length == 0){
      ...
      var token = utils.generateSessionToken(results[0].id, username);
